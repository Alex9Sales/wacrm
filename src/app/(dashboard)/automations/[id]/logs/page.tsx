...
import { getAutomationWithLogs } from "./actions"
import type {
  Automation,
  AutomationLog,
...
export default function AutomationLogsPage({
  ...
  useEffect(() => {
    async function load() {
      try {
        const { automation: aut, logs: logRows } = await getAutomationWithLogs(id)
        setAutomation(aut)
        setLogs(logRows)
      } catch (err) {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="mt-0.5 text-sm text-muted-foreground">Registro de execuções</p>
        </div>
      </div>
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="truncate text-xs text-muted-foreground">
                      {log.trigger_event} · {log.steps_executed?.length ?? 0} etapa{log.steps_executed?.length === 1 ? "" : "s"}
                    </div>
                  </div>
