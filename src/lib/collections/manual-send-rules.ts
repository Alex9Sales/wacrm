 ...
 * régua (sem `kind`) ou outra manual. Lembrete a vencer e aviso de cobrança
 * nova são entrega de link de outra parcela — expirar o aviso perderia o link
 * para sempre (ele é marcado "avisado" em qualquer status).
 */
export function manualCollectExpiresKind(kind: unknown): boolean {
  return kind !== 'reminder' && kind !== 'new_charge'
}
