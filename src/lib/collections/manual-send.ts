...
export async function sendManualCollectCore(accountId: string, userId: string, input: ManualCollectSendInput): Promise<ManualCollectOutcome<ManualCollectSent>> {
  ...
  try {
    ...
    //    lembrete e aviso de cobrança nova são outra parcela (manualCollectExpiresKind).
    await db
      ...
      .where(
        and(
          ...
          sql`${agentActionRequests.payload}->>'kind' IS DISTINCT FROM 'reminder' AND ${agentActionRequests.payload}->>'kind' IS DISTINCT FROM 'new_charge'`,
        ),
      )
  } catch (err) {
    recorded = false
